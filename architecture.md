...
| **Infrastructure** | `src/core/infrastructure` | **HttpClient**, **Api*Repository** classes, DI **container**, token/Pusher helpers. | Knows URLs, headers, cookies, Axios. |
| **Presentation** | `src/core/presentation` | **React hooks** (`useAuth`, `useCustomerManagement`, …) that hold UI-related state and call services. | Components import hooks, not repositories. |

**Supporting code (not “layers” but shared):**

- `src/lib/` — cookies, i18n, small utilities used by infrastructure or hooks.
- `src/config/` — environment-driven config (e.g. Pusher).
...
- **Pusher** — optional realtime; config in `src/config/pusher.ts`.
...
- **Reuse patterns:** Before inventing a new structure, find the closest existing feature (e.g. customers or users) and mirror it.
